...
function trocaImagemCupomMenuMobile() {
  ...
  const existsElement = document.querySelector("#bottom-navigation > div:nth-child(6) > span > svg")
  
  if (!existsElement) return
  newImage.src = "https://static.springbuilder.site/fs/userFiles-v2/7games-18751367/media/ticket.png?1668004946835";
  
  existsElement?.remove();
  
  document.querySelector("#bottom-navigation > div:nth-child(6) > span").appendChild(newImage);
}

function resolveAfter2Seconds() {
  if(document.querySelector("#bottom-navigation #button-menu") !== null) return;
  return new Promise(resolve => {
    setTimeout(() => {
      resolve(document.querySelector("#bottom-navigation > div:last-child").parentNode);
    }, 400);
  });
}

async function trocaBotaoMobile() {
  if(document.querySelector("#bottom-navigation > div:last-child") === null) return;
  if(document.querySelector("#bottom-navigation #button-menu") !== null) return;
  if(document.querySelector("#bottom-navigation #button-chat") !== null) return;
  
  const menuMobile = document.querySelector("#bottom-navigation > div:last-child")
  ...
  const btnMenuExiste = document.querySelector("#bottom-navigation #button-menu")
  const btnChatExiste = document.querySelector("#bottom-navigation #button-chat")
  ...
  document.getElementById("chat-button-bottom").addEventListener("click", function(event){
...
function loadAlertaCupom() {
  $('.style__RadioButton-sc-3ucvic-2, .style__MarketButton-sc-3h3jba-6').off('click');
  $(".style__RadioButton-sc-3ucvic-2, .style__MarketButton-sc-3h3jba-6").click(function () {
    cupomAnima1();
...
function lateralCustomMenu() {
  ...
  var html = `
  <div> 
  ${isMobile ? `<div class="bg-navigation"><div class="position-button">${html_trigger}</div></div>` : html_trigger}
  `;
  for (var i = 0; i < MENUS.length; i++) {
    html +=
  `<a class="menu-item" id="${i}" onClick="clickMenuSelect(${i})" href="${MENUS[i].link}">
      <div>${MENUS[i].svgIcon}</div>
      <span>${MENUS[i].label}</span>  
    </a>`
  }
  
  if(isMobile){html += `<div id="language-switcher-container"></div>`;}
  html += `
...
waitForElm("#lateral-menu").then((elm) => {
  ...
  if(isMobile) {
    document.querySelector( '#language-switcher-container' ).insertAdjacentHTML("afterend", document.querySelector(".blank-tradutor .ModuleLanguageSwitcher").innerHTML);
  }
...
if(!isMobile){
  document.addEventListener("DOMContentLoaded", function() {
    $(".main-rows, header").click(function(event) {
      ...
      if( document.querySelector("#lateral-menu").classList.contains('active') ){
...
function insereCabecalhoW1() {
  ...
  arrSeletores.forEach(function (currentValue, index, arr) {
      ...
      if (document.querySelector(currentValue + ' .match-card__container div[class^="style__Header-sc-"]') == null) { return; }
      if (document.querySelectorAll(currentValue + ' .match-card__container #w1w2').length > 0) { return; }

      document.querySelector(currentValue + ' .match-card__container div[class^="style__Header-sc-"]')
  ...
  if (document.querySelector('#prematch-left-sidebar-wrapper > div > div.lazyLoad__wrapper') != null) {
      if (document.querySelectorAll('#prematch-left-sidebar-wrapper > div > div.lazyLoad__wrapper #w1w2slider').length <= 0 &&
          document.querySelector('#prematch-left-sidebar-wrapper > div > div.lazyLoad__wrapper #w1w2slider') == null) {

          document.querySelector('#prematch-left-sidebar-wrapper > div > div.lazyLoad__wrapper > div:nth-child(1) > div.liveGame__wrapper > a > div  div[class^="style__Header-sc-"]')
              .insertAdjacentHTML('afterend', `<div id="w1w2slider" style="width: 100%;padding-left: 57%;display: inline-flex;height: 25px;margin: auto;/* border: solid 1px red; */ margin-top: -24px; float:right; padding-right: 7px;">
                    <div style="width: 33.33%;text-align:center;color: #fff;margin: auto;">1</div>
                    <div style="width: 33.33;text-align:center;color: #fff;margin: auto; /*margin-left:-12px;*/ ">X</div>
                    <div style="width: 33.33%;text-align:center;color: #fff;margin: auto;">2</div>
                  </div>` );
...
waitForElm(".x-casinoGameCardImageWrapper__image").then((elm) => {
  if(document.querySelectorAll('#m753 .x-casinoGamesByCategories__categoryName svg').length > 0 || document.querySelectorAll('#m875 .x-casinoGamesByCategories__categoryName svg').length > 0) return
  setTimeout("addIconeComponents()", 500);
})

waitForElm(".x-casinoGameCardImageWrapper__image").then((elm) => {
  if(document.querySelectorAll('#m871 .uc-content span svg').length > 0 || document.querySelectorAll('#m848 .uc-content span svg').length > 0) return
  console.log('entrei 1')
  setTimeout("addIconeComponentMaisJogados()", 500);
})

function addIconeComponentMaisJogados() {
  const component = isMobile ? document.querySelector('#m871 .uc-content span') : document.querySelector('#m848 .uc-content span')
...
function addIconeComponents() {
  const component = isMobile ? document.querySelectorAll('#m875 .x-casinoGamesByCategories__categoryName') : document.querySelectorAll('#m753 .x-casinoGamesByCategories__categoryName')
